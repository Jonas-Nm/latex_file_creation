...
class datasheet(Document):
    ...
    def general_settings():
        ...
        text_command()
        banner_command()
        logo = Picture('Logo', r'C:\Users\Jonas\PycharmProjects\latex_file_creation\images\logo.pdf', [-200, 370], '1.5cm')
        logo.insert()
        Text().toprightcorner()
    def title_text(PM_type, SN, opt, title_opt):
        ...
        if opt != []:
            ...
            inc = 0.0
            for element in opt:
                if element in title_opt:
                    inc = inc - 0.015
                    Text(r'- '+title_opt[element], '', [0.5, 0.71+inc]).insert()
